...
function initCardContent() {
    fetchAsJson("/static/resources/json/indexcard.json").then((j) => {
        j.cards.forEach((jElem, ind) => {
            ...
            jElem.tags.forEach((tag) => {
                card.querySelector(
                    ".optional>.tags"
                ).innerHTML += `<li class="tag-${tag}">${tag}</li>`;
            });
            card.onclick = () => {
...
function loadingRecentArticle() {
    fetchAsJson("/static/resources/json/information.json").then((j) => {
        setArticle(j.recent);
    });
}
...
function setArticle(loadingArticlePath) {
    ...
    fetchAsText(
        `/articles/${loadingArticlePath.slice(0, 6)}/${loadingArticlePath}.md`
    ).then((t) => {
...
function initContent(loadingFile, query, callback = () => {}) {
    ...
    fetchAsText(loadingFile).then((t) => {
...
function initTransition() {
    document
        .querySelector("#transition-picture")
    ...
    document
        .querySelector("#transition-software")
        .addEventListener("click", (ev) => {
            transition(softwareViewPath);
            menuCloseIfShown();
        });
    document
        .querySelector("#transition-social")
        .addEventListener("click", (ev) => {
            transition(socialViewPath);
            menuCloseIfShown();
        });
    document
        .querySelector("#transition-blog")
        .addEventListener("click", (ev) => {
